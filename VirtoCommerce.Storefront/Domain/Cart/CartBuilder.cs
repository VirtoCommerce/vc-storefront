    ...
    public class CartBuilder : IWishlistBuilder
    {
        ...
        protected virtual async Task PrepareCartAsync(ShoppingCart cart, Store store)
        {
            ...
            if (cart.Items.Any())
            {
                ...
                var products = await _catalogService.GetProductsAsync(productIds, ItemResponseGroup.ItemWithPrices | ItemResponseGroup.ItemWithDiscounts | ItemResponseGroup.Inventory | ItemResponseGroup.ItemWithDerivativeInfo);
